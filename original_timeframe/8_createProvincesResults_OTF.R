...
ForecastFiles <- list.files("~/.../provinces_forecasts")
#Overview
summary(ForecastFiles)#34 provinces
...
for (i in 1:length(ForecastFiles)) {
  forecast <- read_rds(paste0("~/.../provinces_forecasts/",ForecastFiles[i])) #Reading province's data)
...
for (i in 1:length(ForecastFiles)) {
  forecast <- read_rds(paste0("~/.../provinces_forecasts/",ForecastFiles[i])) #Reading district's data)
  CalDat <- forecast[88:135, ] #Reading 'test data' forecast data into CalDat
  #Calculating 'predicted-n' data
...
##Checking results
table(summary(ErrorsData3$Pre_Nai)) #42 TRUE, 6 FALSE
#(42*100)/48=87.5%

write_rds(ErrorsData3, "~/.../provinces_results_table_OTF.rds")
