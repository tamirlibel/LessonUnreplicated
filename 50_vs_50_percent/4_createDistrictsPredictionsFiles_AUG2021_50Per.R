...
if(!dir.exists("~/.../50_percents_vs_50_percents/districts_forecasts")) dir.create("~/.../50_percents_vs_50_percents/districts_forecasts")

##Creating a list of the files' names in subfolder 'Districts_violent_events_rds'
DisFilesNames <- list.files("~/.../50_percents_vs_50_percents/districts_violent_events_rds")
##Counting districts
...
for (i in 1:length(DisFilesNames)) {
  ...
  district <- read_rds(paste0('~/.../50_percents_vs_50_percents/districts_violent_events_rds/',DisFilesNames[i])) #Reading district's data 
  ...
  for (i in start:length(ObsVec)) {
    ...
    y_hat <- forecast(fit, h=1) #Forecasting for 05/2008
    DatesEvents[start+1, 3] <- y_hat$mean[1]
    start <- start + 1
  }
  initial <- 160 #Defining starting value-08/2012
  ...
  write_rds(DatesEvents, paste0("~/.../50_percents_vs_50_percents/districts_forecasts/forecasts", x, ".rds"))
  setTxtProgressBar(pb, i)
}

##Creating a list of files in districts_forecasts
forecasts_files_names <- list.files("~/.../50_percents_vs_50_percents/districts_forecasts")
