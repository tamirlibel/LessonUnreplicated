...
#Cutting point for test set --> Row 211, 07/2012| ration:2/3 initial training set, 1/3 test set
country_pre <- read_rds("~/.../third_vs_two_thirds/violent_country_all_years_third.rds")  #Reading country's data 
...
for (i in start:length(ObsVec)) {
  ...
  y_hat <- forecast(fit, h=1) #Forecasting for 08/2012
...
for (i in initial:length(ObsVec)) {
  ...
  y_hat <- forecast(fitN, h=1) #Forecasting for 08/2012
  DatesEvents[initial+1, 4] <- y_hat$mean[1]
  initial <- initial + 1
}
DatesEvents <- round(DatesEvents, digits = 0)
write_rds(DatesEvents, paste0("~/.../third_vs_two_thirds/country_forecasts_third", ".rds"))
